...
' Code from: http://www.cpearson.com/excel/SortingArrays.aspx
'   InputArray is the array to be sorted. LB is the first element of the input array to sort.
'   A value of -1 indicates to start sorting with the first element. UB is the last element
'   of the input array to sort. A value of -1 indicates to sort to the end of the array.
'   By modifying the values of LB and UB, you can sort only a subset of the array.
'   Descending, if False or omitted, causes the sort to progress in ascending order.
'   If Descending is True, the array is sorted in descending order. CompareMode indicates
'   whether the sorting is case sensitive or case insensitive. NoAlerts if True, supresses
'   error messages that may occur. The function returns True if the sort was successful or
'   False if an error occurred. There are several procedures that support the QSortInPlace
